 ...
 */
  ...
  */
  init: function(options) {
		...
		self.outputFilePath = options.outputFilePath;
		self.outputFileName = options.outputFileName;

		// Moving on...
		self.getFile(self.inputFileName);
	},
  /**
  * Gets the specified input file and reads the content.
  * @function getFile
	* @param {string} inputFileName - file name to get CSS from.
  * @memberof CombineMq
  */
  getFile: function (inputFileName) {
		var self = this; // Scope resolution

		// Read file in
		fs.readFile(inputFileName, 'utf8', function(err, fileContents) {
			if(err) {
				// Write error messages
				console.log(chalk.red(dammit({NSFW: false})));
				console.log(chalk.red('(Could not open file: "' + inputFileName + '")'));
				// Exit out
				process.exit(1);
			}
			else {
				console.log(chalk.bold('\nCombining file: ' + inputFileName + '...'));
				// Moving on...
  ...
  parseFile: function (contents) {
    ...
    /**
  	...
  	* @memberof CombineMq
      ...
      // Create a dir
      mkdirp(self.outputFilePath, function(err) {
      	if(err) {
      		throw err;
      	}
      	else {
      		process.chdir(self.outputFilePath);
      		fs.writeFile(self.outputFileName + '.css', fileContents, function (err) {
      			if (err) {
      				throw err;
      			}

      			// Write a succes message
      			var sup = superb();
