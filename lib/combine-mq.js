 ...
 module.exports = {
	 ...
	 */
	 parseCssString: function(contents, options) {
		...
		// Do the processing
		var cssJson = parseCss(contents, {source: options.src}),
		strStyles = '',
		processedCSS = {};
...
var i = 0;
		...
		// Function to output container queries
		var outputContainer = function(container) {
			container.forEach(function(item) {
				strStyles += processContainer(item);
			});
		};
		...
		// Check if container queries were processed
		if (processedCSS.container.length !== 0) {
			outputContainer(processedCSS.container);
		}
